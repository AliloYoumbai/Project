...
public class LoginServlet3 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Jamia jam = new Comand().getJamia(email);

		String bdpassword = jam.getPassword();
		if (password.equals(bdpassword)) {
			HttpSession session = request.getSession(true);
			session.setAttribute("bdnamej", jam);

			this.getServletContext().getRequestDispatcher("/ProfileServlet2").forward(request, response);
